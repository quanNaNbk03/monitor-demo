...
func NewPrometheusClient() (PrometheusRepository, error) {
	c, err := client.NewClient(viper.GetString("monitorMetric.address"), viper.GetDuration("monitorMetric.scrapeInterval"))
	if err != nil {
...
func (c *Client) QueryRange(ctx context.Context, metricByTypeConfig model.MetricByTypeConfig, req *model.MonitorByTypeParam) (*model.Monitor, *ierror.CoreError) {
	...
	for _, target := range metricByTypeConfig.Targets {
		...
		for _, targetTemp := range targetTemps {
			...
			monitorResponseTarget := convertMatrixValue(targetTemp, modelValue)
...
func convertMatrixValue(m model.MetricByTypeConfigTarget, value prometheusmodel.Value) *model.MonitorTarget {
